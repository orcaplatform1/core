  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <ToolCard title="Blok Yüksekliği" icon={Blocks} accent="primary">
        ...
        <p className="mt-1 text-xs text-muted-foreground">Bitcoin ana zinciri</p>
      </ToolCard>
        ...
        {data.mempool ? (
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{data.mempool.vsizeMb.toFixed(1)} MB bekliyor</p>
            </div>
          </div>
        ) : (
          <p className="text-xs text-muted-foreground">Veri yükleniyor...</p>
        ...
        {data.fees ? (
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="rounded-lg card-inner p-2.5">
              ...
              <p className="mt-0.5 text-base font-bold text-foreground">{data.fees.fastestFee}</p>
            </div>
            <div className="rounded-lg card-inner p-2.5">
              <p className="text-muted-foreground">30 Dakika</p>
              <p className="mt-0.5 text-base font-bold text-foreground">{data.fees.halfHourFee}</p>
            </div>
            <div className="rounded-lg card-inner p-2.5">
              <p className="text-muted-foreground">1 Saat</p>
              <p className="mt-0.5 text-base font-bold text-foreground">{data.fees.hourFee}</p>
            </div>
            <div className="rounded-lg card-inner p-2.5">
              <p className="text-muted-foreground">Ekonomik</p>
              <p className="mt-0.5 text-base font-bold text-foreground">{data.fees.economyFee}</p>
            </div>
          </div>
        ) : (
          <p className="text-xs text-muted-foreground">Veri yükleniyor...</p>
      ...
      <ToolCard title="Hash Rate & Zorluk" icon={Activity} accent="purple">
        {data.hashrate ? (
          <>
            <p className="text-2xl font-bold text-foreground">{data.hashrate.currentEh.toFixed(1)} EH/s</p>
      ...
      <ToolCard title="Aktif Adres (24s)" icon={Users} accent="primary">
        <p className="text-2xl font-bold text-foreground">{fmtCompact(data.activeAddresses24h)}</p>
        <p className="mt-1 text-xs text-muted-foreground">Kaynak: Blockchain.com</p>
      </ToolCard>

      <ToolCard title="İşlem Hacmi (24s)" icon={Activity} accent="warning">
        <p className="text-2xl font-bold text-foreground">{fmtCompact(data.txVolume24h)}</p>
        <p className="mt-1 text-xs text-muted-foreground">Kaynak: Blockchain.com</p>
      </ToolCard>
    </div>
  );
