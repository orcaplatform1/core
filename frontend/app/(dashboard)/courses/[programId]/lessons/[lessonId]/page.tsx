      ...
      <div className="flex flex-col gap-3 rounded-xl border border-border bg-card px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <span className="shrink-0 text-xs text-muted-foreground">Modül İlerlemesi</span>
            ...
            <span className="shrink-0 text-xs font-semibold text-foreground">%{moduleProgressPercent}</span>
          </div>
          {!lesson.locked && (
            <div className="flex items-center gap-2">
              <Button
      ...
      <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="flex min-w-0 flex-col gap-4">
          <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <h1 className="min-w-0 truncate font-semibold text-foreground">{lesson.title}</h1>
                  ...
                  <div className="rounded-2xl border border-border bg-card p-4">
                    <div className="flex max-h-80 flex-col gap-1 overflow-y-auto">
                      {moduleLessons.length === 0 && (
                        <p className="text-sm text-muted-foreground">Bu modülde henüz ders yok.</p>
                      )}
                      {moduleLessons.map((l) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={`min-w-0 flex-1 truncate ${active ? "font-medium text-primary" : "text-foreground"}`}>
                              {l.title}
                            </span>
                            {l.durationSeconds && (
              ...
              <TabsContent value="resources" className="mt-4">
                <div className="rounded-2xl border border-border bg-card p-4 sm:p-6">
                  {lesson.resources.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Bu ders için kaynak eklenmemiş.</p>
                  ) : (
