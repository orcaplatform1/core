    ...
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-h2 text-[#F5F1EA]">Referans Kazanç İstatistikleri</h2>
        </div>
      ...
      {isLoading ? (
        ...
        <p className="mt-4 text-body-sm text-[#A8A6A0]">Bu dönemde referans dönüşümü yok.</p>
      ) : (
        <>
          <div className="mt-4 grid grid-cols-2 gap-3 sm:w-72">
            <div className="rounded-xl bg-card-inner p-3">
              <p className="text-body-xs text-[#A8A6A0]">Dönüşen Referans</p>
              <p className="mt-1 text-num-md text-[#F5F1EA]">{totalConversions}</p>
            </div>
            <div className="rounded-xl bg-card-inner p-3">
              <p className="text-body-xs text-[#A8A6A0]">Dağıtılan Kredi</p>
              <p className="mt-1 text-num-md text-[#F5F1EA]">{totalCredits}</p>
            </div>
          </div>
            ...
            <table className="w-full text-table-cell">
              <thead>
                <tr className="border-b border-border text-left text-table-header text-[#A8A6A0]">
                  ...
                  <th className="px-3 py-2">Dağıtılan Kredi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {[...rows].reverse().map((r) => (
                  <tr key={r.period}>
                    ...
                    <td className="px-3 py-2 text-financial text-[#F5F1EA]">{r.creditsAwarded}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ReferrerInvitees({ userId }: { userId: string }) {
  ...
  if (isLoading) return <p className="px-4 pb-4 text-body-xs text-[#A8A6A0]">Yükleniyor...</p>;
  if (!invitees || invitees.length === 0) {
    return <p className="px-4 pb-4 text-body-xs text-[#A8A6A0]">Henüz davet ettiği kimse yok.</p>;
  }
  return (
    <div className="border-t border-border px-4 py-3">
      <p className="mb-2 text-body-xs text-[#A8A6A0]">Davet ettiği üyeler ({invitees.length})</p>
      <div className="space-y-1.5">
        {invitees.map((inv) => (
          <div key={inv.id} className="flex items-center justify-between rounded-lg bg-card-inner px-3 py-2 text-body-sm">
            <div>
              <Link href={`/manage/users/${inv.id}`} className="text-[#F5F1EA] hover:text-primary hover:underline">
                {inv.fullName}
              </Link>
              <p className="text-body-xs text-[#A8A6A0]">@{inv.username ?? "—"} · {inv.email ?? "email yok"}</p>
            </div>
            <p className="text-body-xs text-[#A8A6A0]">{new Date(inv.createdAt).toLocaleDateString("tr-TR")}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-h1 text-[#F5F1EA]">Davetiye Sistemi</h1>
          <p className="text-body-sm text-[#A8A6A0]">Üye referansları, kazandıkları kredi ve davet ettikleri kişiler.</p>
        </div>
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-border bg-card p-8 text-center space-y-2">
            ...
            <p className="text-body-sm text-[#A8A6A0]">Henüz kimse referans koduyla üye davet etmemiş.</p>
          </div>
        ) : (
          referrers.map((r) => {
                ...
                >
                  <div>
                    <Link
                      ...
                      className="text-card-title-sm text-[#F5F1EA] hover:text-primary hover:underline"
                    >
                      {r.fullName}
                    </Link>
                    <p className="text-body-xs text-[#A8A6A0]">Referans kodu: @{r.username ?? "—"}</p>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className="text-body-xs text-[#A8A6A0]">Davet ettiği</p>
                      <p className="text-num-sm text-[#F5F1EA]">{r.invitedCount}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-body-xs text-[#A8A6A0]">Kazandığı kredi</p>
                      <p className="text-num-sm text-[#22C55E]">{r.creditsEarned}</p>
                    </div>
                    {expanded ? (
