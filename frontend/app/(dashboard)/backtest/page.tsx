...
export default function BacktestPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-h1 text-[#F5F1EA]">Backtest</h1>
            ...
            <div className="flex flex-wrap gap-2">
              {(category === "crypto" ? symbols?.crypto : symbols?.forex)?.map((s) => (
                <button
                  key={s}
                  onClick={() => setSymbol(s)}
                  className="rounded-xl border border-border bg-card-inner px-4 py-2 text-body-sm text-[#A8A6A0] hover:border-primary hover:text-[#F5F1EA]"
                >
                  {s}
                </button>
              ))}
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <h2 className="text-card-title-md text-[#F5F1EA]">{symbol}</h2>
              <div className="flex rounded-xl border border-border overflow-hidden">
                <button
                  onClick={() => setTimeframe("1d")}
                  className={`px-3 py-1.5 text-body-sm ${timeframe === "1d" ? "bg-primary text-white" : "bg-card-inner text-[#A8A6A0]"}`}
                >
                  Günlük
                </button>
                <button
                  onClick={() => setTimeframe("1h")}
                  className={`px-3 py-1.5 text-body-sm ${timeframe === "1h" ? "bg-primary text-white" : "bg-card-inner text-[#A8A6A0]"}`}
                >
                  1 Saatlik
                </button>
              </div>
