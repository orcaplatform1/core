...
const INTERVAL = '1d';
const LIMIT = 1000;
const BATCHES = 2; // ~2000 gün (yaklaşık 5.5 yıl)
...
async function fetchKlines(symbol: string, endTime?: number) {
...
async function seedSymbol(symbol: string) {
  console.log(`${symbol} verisi çekiliyor...`);

  let allCandles: any[] = [];
  let endTime: number | undefined = undefined;

  for (let i = 0; i < BATCHES; i++) {
    const batch = await fetchKlines(symbol, endTime);
  ...
  if (allCandles.length === 0) {
    console.log(`${symbol}: Binance'de bulunamadı, atlanıyor.`);
    return;
  }

  console.log(`${symbol}: ${allCandles.length} mum bulundu, kaydediliyor...`);
...
async function main() {
  ...
  for (const symbol of symbols) {
    ...
    await new Promise((r) => setTimeout(r, 300)); // Binance rate limit için bekleme
  }
